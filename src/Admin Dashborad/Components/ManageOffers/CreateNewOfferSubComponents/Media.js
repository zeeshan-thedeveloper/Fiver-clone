...
import { selectMediaInfoOfCreateNewOffer } from '../Redux Components/Selectors';
import { updateMediaOfCreateNewOffer } from '../Redux Components/offersManagerSlice';
function Media(props) {
    ...
    const mediaFromStore = useSelector(selectMediaInfoOfCreateNewOffer);
    const dispatch = useDispatch();
    useEffect(()=>{
        //Load data from store into hooks
        setThumbnailImage(mediaFromStore.offerImage);
    ...
    const handelEditAndSaveChanges = ()=>{
        if(!isEditingEnabled)
        {
            ...
            const payload = {
                offerImage:thumbnailImage,
                isEditingEnabled: true
            }
            dispatch(updateMediaOfCreateNewOffer(payload));
        }
        else
        {
