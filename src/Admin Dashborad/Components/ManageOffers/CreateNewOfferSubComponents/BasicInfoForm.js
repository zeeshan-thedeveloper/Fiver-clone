...
import { selectBasicInfoOfCreateNewOffer } from '../Redux Components/Selectors';
import {updateBasicInfoOfCreateNewOffer} from '../Redux Components/offersManagerSlice'
function BasicInfoForm(props) {
    const [isEditingEnabled,setIsEditingEnabled]=useState(false);
    //Data hooks
    const [offerPunchLink,setofferPunchLink]=useState();
    ...
    const basicInfoFromStore = useSelector(selectBasicInfoOfCreateNewOffer);
    const dispatch = useDispatch();
    useEffect(()=>{
        //load data into hooks from store.
        setofferPunchLink(basicInfoFromStore.offerPunchLine);
        ...
        setCatagory(basicInfoFromStore.offerCatagory);
        setSubCatagory(basicInfoFromStore.offerSubCatagory)
    ...
    const handelEditAndSaveChanges = ()=>{
        ...
        if(!isEditingEnabled)
        {
            ...
            const data = {
                offerPunchLine:offerPunchLink,
                offerCreationDate:'3-2-2019',
                offerCreationTime:"4:00 PM",
                offerCatagory:catagory,
                offerSubCatagory:subCatagory,
                isEditingEnabled: true
            } 
            dispatch(updateBasicInfoOfCreateNewOffer(data));
        }
        else
        {
            ...
            <CardContent>
                <Grid container>
                    ...
                    <Grid item xs={10}>
                        ...
                        fontSize={20} 
                        fontWeight={'bold'}
                        label={"Offer Punch line"} 
                        width={'40rem'}
                        height={15}
                        value={offerPunchLink} 
                        setValue={setofferPunchLink}
