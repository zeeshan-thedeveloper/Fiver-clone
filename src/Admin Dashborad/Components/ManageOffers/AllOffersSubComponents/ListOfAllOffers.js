...
import { selectAll, selectListOfAllOffers } from '../Redux Components/Selectors';
import UserAccountDataHolderForList from './OfferDataHolderForList';
function ListOfAllOffers({showMenueSelectionOpt=false,...props}) {
    const classes = useStyles();
    const [listOfAllOffers,setlistOfAllOffers]=useState([]);
    ...
    const filterBy_titles_type={
        ...
        filter_by_title_1_type:"offerSubCatagory",
        filter_by_title_2_type:"offerCatagory",
        filter_by_title_3_type:"projectRatingStars",
        filter_by_title_4_type:"offerCreationDate",
        filter_by_title_5_type:"numberOfHits",
        filter_by_title_6_type:"offerStatus",
        filter_by_title_7_type:"offerId",
        
    }

    const listOfAllOffersFromStore = useSelector(selectListOfAllOffers); //this will be updated and will cause auto render as soon as sotre get updated.
    const {isLoading_loadDataOfListOfAllOffers} = useSelector(selectAll);

    useEffect(()=>{
       // setting up list of projects in project's rneder list.
       setlistOfAllOffers(listOfAllOffersFromStore);
       setNumberOfPage(Math.ceil((listOfAllOffersFromStore.length/9)))
    },[listOfAllOffersFromStore]);

    useEffect(()=>{
       updateList(startIndexOfPage,(listOfAllOffers.length>=endIndexOfPage) ? endIndexOfPage : listOfAllOffers.length);
    },[listOfAllOffers,endIndexOfPage]);
    ...
    useEffect(()=>{
        ...
        try{
            const searchResultsList = FilterEngine(listOfAllOffers,searchKeyPairs[0]);
...
const updateList = (startIndex,endIndex)=>{
        setList(listOfAllOffers.map((item,index)=>{
            ...
            <Grid container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        <CardContent>
                            {
                                (isLoading_loadDataOfListOfAllOffers) ? (
                                    <div>
                                    <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Fetching All Offers.."} fontSize={30}/> </div>
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                            ...
                            <div>
                                { (listOfOptions_ForChipList.length===0) && 
                                    <div>
                                        ...
                                        <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"All Offers"} fontSize={30}/> </div>
