...
import { selectAll, selectListOfCanceledOrder, selectListOfCompletedOrders } from '../Redux Components/Selectors';
import OrderInfoHolderForList from './OrderInfoHolderForList';
function ListOfCompletedOrders({showMenueSelectionOpt=false,...props}) {
    const classes = useStyles();
    const [listOfCompletedOrders,setlistOfCompletedOrders]=useState([]);
    ...
    const listOfCompletedOrderFromStore = useSelector(selectListOfCompletedOrders); //this will be updated and will cause auto render as soon as sotre get updated.
    const {isLoading_LoadListOfCompletedOrders} = useSelector(selectAll);

    useEffect(()=>{
       // setting up list of projects in project's rneder list.
       setlistOfCompletedOrders(listOfCompletedOrderFromStore);
       setNumberOfPage(Math.ceil((listOfCompletedOrderFromStore.length/9)))
    },[listOfCompletedOrderFromStore]);

    useEffect(()=>{
       updateList(startIndexOfPage,(listOfCompletedOrders.length>=endIndexOfPage) ? endIndexOfPage : listOfCompletedOrders.length);
    },[listOfCompletedOrders,endIndexOfPage]);
    ...
    useEffect(()=>{
        ...
        try{
            const searchResultsList = FilterEngine(listOfCompletedOrders,searchKeyPairs[0]);
...
const updateList = (startIndex,endIndex)=>{
        setList(listOfCompletedOrders.map((item,index)=>{
            ...
            <Grid container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        <CardContent>
                            {
                                (isLoading_LoadListOfCompletedOrders) ? (
                                    <div>
                                    <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Fetching Completed orders.."} fontSize={30}/> </div>
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                            ...
                            <div>
                                { (listOfOptions_ForChipList.length===0) && 
                                    <div>
                                        ...
                                        <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Completed orders"} fontSize={30}/> </div>
