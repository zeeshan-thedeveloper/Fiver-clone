...
import { selectAll, selectListOfInRevisionOrder } from '../Redux Components/Selectors';
import OrderInfoHolderForList from './OrderInfoHolderForList';
function ListOfInRevisionOrders({showMenueSelectionOpt=false,...props}) {
    ...
    const listOfInRevisionOrderFromStore = useSelector(selectListOfInRevisionOrder); //this will be updated and will cause auto render as soon as sotre get updated.
    const {isLoading_LoadListOfInRevsionOrder} = useSelector(selectAll);

    useEffect(()=>{
       // setting up list of projects in project's rneder list.
       setlistOfNewOrders(listOfInRevisionOrderFromStore);
       setNumberOfPage(Math.ceil((listOfInRevisionOrderFromStore.length/9)))
    },[listOfInRevisionOrderFromStore]);
            ...
            <Grid container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        <CardContent>
                            {
                                (isLoading_LoadListOfInRevsionOrder) ? (
                                    <div>
                                    <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Fetching In-Revision orders.."} fontSize={30}/> </div>
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                            ...
                            <div>
                                { (listOfOptions_ForChipList.length===0) && 
                                    <div>
                                        ...
                                        <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"In-Revision orders"} fontSize={30}/> </div>
...
export default ListOfInRevisionOrders;