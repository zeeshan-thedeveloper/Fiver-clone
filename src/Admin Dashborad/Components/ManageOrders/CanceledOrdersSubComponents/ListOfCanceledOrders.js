...
import { selectAll, selectListOfCanceledOrder, selectListOfInProgressOrders } from '../Redux Components/Selectors';
import OrderInfoHolderForList from './OrderInfoHolderForList';
function ListOfCanceledOrders({showMenueSelectionOpt=false,...props}) {
    const classes = useStyles();
    const [listOfCanceledOrders,setlistOfCanceledOrders]=useState([]);
    ...
    const listOfCanceledOrderFromStore = useSelector(selectListOfCanceledOrder); //this will be updated and will cause auto render as soon as sotre get updated.
    const {isLoading_LoadListOfCanceledOrder} = useSelector(selectAll);

    useEffect(()=>{
       // setting up list of projects in project's rneder list.
       setlistOfCanceledOrders(listOfCanceledOrderFromStore);
       setNumberOfPage(Math.ceil((listOfCanceledOrderFromStore.length/9)))
    },[listOfCanceledOrderFromStore]);

    useEffect(()=>{
       updateList(startIndexOfPage,(listOfCanceledOrders.length>=endIndexOfPage) ? endIndexOfPage : listOfCanceledOrders.length);
    },[listOfCanceledOrders,endIndexOfPage]);
    ...
    useEffect(()=>{
        ...
        try{
            const searchResultsList = FilterEngine(listOfCanceledOrders,searchKeyPairs[0]);
...
const updateList = (startIndex,endIndex)=>{
        setList(listOfCanceledOrders.map((item,index)=>{
            ...
            <Grid container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        <CardContent>
                            {
                                (isLoading_LoadListOfCanceledOrder) ? (
                                    <div>
                                    <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Fetching Canceled orders.."} fontSize={30}/> </div>
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                            ...
                            <div>
                                { (listOfOptions_ForChipList.length===0) && 
                                    <div>
                                        ...
                                        <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Canceled orders"} fontSize={30}/> </div>
...
export default ListOfCanceledOrders;