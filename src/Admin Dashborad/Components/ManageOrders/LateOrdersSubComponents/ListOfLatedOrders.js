...
import { selectAll, selectListOfLateOrder } from '../Redux Components/Selectors';
import OrderInfoHolderForList from './OrderInfoHolderForList';
function ListOfLatedOrders({showMenueSelectionOpt=false,...props}) {
    ...
    const listOfLatedOrderFromStore = useSelector(selectListOfLateOrder); //this will be updated and will cause auto render as soon as sotre get updated.
    const {isLoading_LoadListOfLatedOrder} = useSelector(selectAll);

    useEffect(()=>{
       // setting up list of projects in project's rneder list.
       setlistOfNewOrders(listOfLatedOrderFromStore);
       setNumberOfPage(Math.ceil((listOfLatedOrderFromStore.length/9)))
    },[listOfLatedOrderFromStore]);
            ...
            <Grid container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        <CardContent>
                            {
                                (isLoading_LoadListOfLatedOrder) ? (
                                    <div>
                                    <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Fetching Late orders.."} fontSize={30}/> </div>
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                            ...
                            <div>
                                { (listOfOptions_ForChipList.length===0) && 
                                    <div>
                                        ...
                                        <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Late orders"} fontSize={30}/> </div>
...
export default ListOfLatedOrders;