...
import FiltersForBlocked from '../FiltersForBlocked'
import { lightBorder } from '../../../../Theme/borders';
import { Headings } from '../../Support/Headings';
import Pagination from '@material-ui/lab/Pagination';
import { ScrollView } from "@cantonjs/react-scroll-view";
import FilterEngine from '../FilterMotorBlocked'
import produce from 'immer';
import Skeleton from '@material-ui/lab/Skeleton';
import { useSelector } from 'react-redux';
import { selectAll, selectListOfBlockedUserAccounts } from '../Redux Components/Selectors';
import UserAccountDataHolderForList from './UserAccountDataHolderForList';
function ListOfBlockedUserAccounts({showMenueSelectionOpt=false,...props}) {
    const classes = useStyles();
    const [listOfBlockedUserAccounts,setlistOfBlockedUserAccounts]=useState([]);
    ...
    const listOfBlockedUserAccountsFromStore = useSelector(selectListOfBlockedUserAccounts); //this will be updated and will cause auto render as soon as sotre get updated.
    const {isLoading_LoadListOfBlockedUserAccounts} = useSelector(selectAll);

    useEffect(()=>{
       // setting up list of projects in project's rneder list.
       setlistOfBlockedUserAccounts(listOfBlockedUserAccountsFromStore);
       setNumberOfPage(Math.ceil((listOfBlockedUserAccountsFromStore.length/9)))
    },[listOfBlockedUserAccountsFromStore]);

    useEffect(()=>{
       updateList(startIndexOfPage,(listOfBlockedUserAccounts.length>=endIndexOfPage) ? endIndexOfPage : listOfBlockedUserAccounts.length);
    },[listOfBlockedUserAccounts,endIndexOfPage]);
    ...
    useEffect(()=>{
        ...
        try{
            const searchResultsList = FilterEngine(listOfBlockedUserAccounts,searchKeyPairs[0]);
...
const updateList = (startIndex,endIndex)=>{
        setList(listOfBlockedUserAccounts.map((item,index)=>{
...
const updateSearchResultsList = (startIndex,endIndex,listOfResults)=>{
    ...
    return (
        <div>
            <FiltersForBlocked listOfOptions_ForChipList={listOfOptions_ForChipList} setListOfOptions_ForChipList={setListOfOptions_ForChipList}/>
            <Grid container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        <CardContent>
                            {
                                (isLoading_LoadListOfBlockedUserAccounts) ? (
                                    <div>
                                    <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Fetching Blocked User Accounts.."} fontSize={30}/> </div>
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                            ...
                            <div>
                                { (listOfOptions_ForChipList.length===0) && 
                                    <div>
                                        ...
                                        <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Blocked User Accounts"} fontSize={30}/> </div>
...
export default ListOfBlockedUserAccounts;