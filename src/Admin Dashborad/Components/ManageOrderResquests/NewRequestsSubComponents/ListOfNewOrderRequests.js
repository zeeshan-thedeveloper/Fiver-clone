...
import { selectAll, selectListOfNewOrderRequests } from '../Redux Components/Selectors';
import RequestInfoHolderForList from './RequestInfoHolderForList';
function ListOfNewOrderRequests({showMenueSelectionOpt=false,...props}) {
    const classes = useStyles();
    const [listOfNewOrderRequests,setlistOfNewOrderRequests]=useState([]);
    ...
    const listOfNewOrderRquestsFromStore = useSelector(selectListOfNewOrderRequests); //this will be updated and will cause auto render as soon as sotre get updated.
    const {isLoading_LoadListOfNewOrderRequests} = useSelector(selectAll);

    useEffect(()=>{
       // setting up list of projects in project's rneder list.
       setlistOfNewOrderRequests(listOfNewOrderRquestsFromStore);
       setNumberOfPage(Math.ceil((listOfNewOrderRquestsFromStore.length/9)))
    },[listOfNewOrderRquestsFromStore]);

    useEffect(()=>{
       updateList(startIndexOfPage,(listOfNewOrderRequests.length>=endIndexOfPage) ? endIndexOfPage : listOfNewOrderRequests.length);
    },[listOfNewOrderRequests,endIndexOfPage]);
    ...
    useEffect(()=>{
        ...
        try{
            const searchResultsList = FilterEngine(listOfNewOrderRequests,searchKeyPairs[0]);
...
const updateList = (startIndex,endIndex)=>{
        setList(listOfNewOrderRequests.map((item,index)=>{
            ...
            <Grid container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        <CardContent>
                            {
                                (isLoading_LoadListOfNewOrderRequests) ? (
                                    <div>
                                    <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Fetching New Order Requests.."} fontSize={30}/> </div>
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                            ...
                            <div>
                                { (listOfOptions_ForChipList.length===0) && 
                                    <div>
                                        ...
                                        <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"New Order Requests"} fontSize={30}/> </div>
...
export default ListOfNewOrderRequests;