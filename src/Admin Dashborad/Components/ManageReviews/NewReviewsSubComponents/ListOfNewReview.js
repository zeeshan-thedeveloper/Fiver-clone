...
import { selectAll, selectListOfNewReviews } from '../Redux Components/Selectors';
import ReviewInforHolderForList from './ReviewInforHolderForList';
function ListOfNewReview({showMenueSelectionOpt=false,...props}) {
    const classes = useStyles();
    const [listOfNewReviews,setlistOfNewReviews]=useState([]);
    ...
    const listOfNewReviewsFromStore = useSelector(selectListOfNewReviews); //this will be updated and will cause auto render as soon as sotre get updated.
    const {isLoading_loadListOfNewReviews} = useSelector(selectAll);
 
    useEffect(()=>{
       // setting up list of projects in project's rneder list.
       setlistOfNewReviews(listOfNewReviewsFromStore);
       setNumberOfPage(Math.ceil((listOfNewReviewsFromStore.length/9)))
    },[listOfNewReviewsFromStore]);

    useEffect(()=>{
       updateList(startIndexOfPage,(listOfNewReviews.length>=endIndexOfPage) ? endIndexOfPage : listOfNewReviews.length);
    },[listOfNewReviews,endIndexOfPage]);
    ...
    useEffect(()=>{
        ...
        try{
            const searchResultsList = FilterEngine(listOfNewReviews,searchKeyPairs[0]);
...
const updateList = (startIndex,endIndex)=>{
        setList(listOfNewReviews.map((item,index)=>{
            ...
            <Grid container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        <CardContent>
                            {
                                (isLoading_loadListOfNewReviews) ? (
                                    <div>
                                    <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Fetching New Reviews.."} fontSize={30}/> </div>
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                            ...
                            <div>
                                { (listOfOptions_ForChipList.length===0) && 
                                    <div>
                                        ...
                                        <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"New Reviews"} fontSize={30}/> </div>
...
export default ListOfNewReview;