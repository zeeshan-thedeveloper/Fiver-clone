...
import { selectAll, selectListOfPublishedReviews } from '../Redux Components/Selectors';
import ReviewInforHolderForList from './ReviewInforHolderForList';
function ListOfPublishedReview({showMenueSelectionOpt=false,...props}) {
    const classes = useStyles();
    const [listOfPublishedReviews,setlistOfPublishedReviews]=useState([]);
    ...
    const listOfPublishedReviewsFromStore = useSelector(selectListOfPublishedReviews); //this will be updated and will cause auto render as soon as sotre get updated.
    const {isLoading_loadListOfPublishedReviews} = useSelector(selectAll);
 
    useEffect(()=>{
       // setting up list of projects in project's rneder list.
       setlistOfPublishedReviews(listOfPublishedReviewsFromStore);
       setNumberOfPage(Math.ceil((listOfPublishedReviewsFromStore.length/9)))
    },[listOfPublishedReviewsFromStore]);

    useEffect(()=>{
       updateList(startIndexOfPage,(listOfPublishedReviews.length>=endIndexOfPage) ? endIndexOfPage : listOfPublishedReviews.length);
    },[listOfPublishedReviews,endIndexOfPage]);
    ...
    useEffect(()=>{
        ...
        try{
            const searchResultsList = FilterEngine(listOfPublishedReviews,searchKeyPairs[0]);
...
const updateList = (startIndex,endIndex)=>{
        setList(listOfPublishedReviews.map((item,index)=>{
            ...
            <Grid container>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        <CardContent>
                            {
                                (isLoading_loadListOfPublishedReviews) ? (
                                    <div>
                                    <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Fetching Published Reviews.."} fontSize={30}/> </div>
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                                    <Skeleton />
                            ...
                            <div>
                                { (listOfOptions_ForChipList.length===0) && 
                                    <div>
                                        ...
                                        <div style={{marginTop:"1%",paddingBottom:'1%'}}> <Headings text={"Published Reviews"} fontSize={30}/> </div>
...
export default ListOfPublishedReview;